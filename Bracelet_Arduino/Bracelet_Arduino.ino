...
SoftwareSerial bluetoothSerial(2,3);//2 violet, 3 blue
...
void readTag(uint16_t timeout) {  
  ...
  uint8_t success = nfc.readPassiveTargetID(PN532_MIFARE_ISO14443A, uid, &uidLength, timeout);
    ...
    //tags_cont.print();
    bluetoothObject.insertTagIntoBuffer(Tag_Data(uid));
    
		Serial.println(F("Saved tags:"));
    bluetoothObject.getJsonArray().prettyPrintTo(Serial);
    Serial.println();
    
    bluetoothObject.getJsonArray().printTo(bluetoothSerial);
